...
lpm <-lm(cook_fl.Dummy ~ Female.Dummy + Poverty.Dummy + HHsize + HHsize_sq+ Poverty.Dummy*HHsize + as.factor(ed_hd), data=cook_data)
summary(lpm, vcov=sandwich)
stargazer(lpm, lpm2, lpm3, lpm4, digits=3, out=paste(output,"T11_reg3_R.html",sep=""))

# logit coefficients
logit <- glm(cook_fl.Dummy ~ Female.Dummy  + Poverty.Dummy + HHsize + HHsize_sq + Poverty.Dummy*HHsize + as.factor(ed_hd), data=cook_data, family='binomial')
summary(logit)
glance(logit)


me <- margins(logit, type = "response") 
...
# probit coefficients
probit <- glm(cook_fl.Dummy ~ Female.Dummy  + Poverty.Dummy + HHsize + HHsize_sq+ Poverty.Dummy*HHsize + as.factor(ed_hd), data=cook_data, family=binomial(link="probit"))
summary(probit)
